...
exports.createUser = async (req, res, next) => {
    ...
    conexion.query(sql, (error, rows)=>{
        if (error) {
            res.json({status: 500, messge: 'ERROR_USER_CREATE'})
        } else {
            res.json({status: 200, messge: 'USER_CREATE_SUCCESFULLY'})
        }
    })
}
...
exports.updateUser = (req, res)=>{
    ...
    conexion.query(sql, (error, rows)=>{
        if (error) {
            ...
            res.json("USER_UPDATE_SUCCESSFULY")
        }
    })
}
...
exports.deleteUser = (req, res)=>{
    ...
    conexion.query(sql, (error, rows)=>{
        if (error) {
            ...
            res.json("USER_DELETED_SUCCESSFULY")
        }
    })
}
